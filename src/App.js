...
import { RécupérationMeteo } from './fonction';
...
function App() {
  ...
  const airQualityIndex = meteo?.current?.air_quality?.['us-epa-index'] || 0;
   ...
   return (
    <div className='relative items-center justify-center bg-[#c1c2c6] p-6 p-6'>
     <div className='min-h-screen w-[1300px]  text-black grid grid-cols-3 rounded-2xl'>
      <div className='col-span-1 text-black bg-[#ffffff] flex-col p-6'>
        ...
        {meteo && meteo.main ? (
          <h1 className="text-white text-center font-italic text-5xl mb-4">
            {meteo.main.temp} °C
          </h1>
        ) : null}
        <p className="text-white m-5 text-center">
          {dateActuelle.toLocaleDateString('fr-FR', { weekday: 'long' })} , 
          {dateActuelle.getHours()}:{dateActuelle.getMinutes().toString().padStart(2, '0')} 
        </p>
      ...
      <div className='relative w-full p-8 col-span-2 bg-[#f7f6f9] flex flex-col'>
        <div className="mb-6">
          <button className='text-gray-400 hover:text-gray-600 font-medium mr-3'>Aujourd'hui</button>
      ...
      <div className='mt-8'>
        <h1 className='text-2xl font-bold text-gray-800 mb-6'>LES TEMPS FORTS D'AUJOURD'HUI</h1>
      ...
      <div className="bg-gradient-to-br from-gray-50 to-white p-6 rounded-2xl">
        ...
        <div className="text-center text-xs text-gray-400 mt-2">Faible 0-2 | Modéré 3-5 | Élevé 6-7</div>
      </div>

      <div className="bg-gradient-to-br from-gray-50 to-white p-6 rounded-2xl">
        <div className="text-gray-600 text-sm mb-4">État du vent</div>
        <div className="text-5xl font-bold text-gray-800 mb-2">
          {Wind_kph ? Wind_kph.toFixed(1) : '7.7'}
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
        ...
        <div className="bg-gradient-to-br from-gray-50 to-white p-6 rounded-2xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>{humidity < 30 ? '😐' : humidity < 60 ? '👍' : '💧'}</span>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-gray-50 to-white p-6 rounded-2xl">
          ...
          <div className="text-5xl font-bold text-gray-800 mb-2">
            {visibility ? visibility.toFixed(1) : '10'} 
          ...
          <div className="flex items-center gap-2">
            ...
            <span>{visibility > 10 ? '😊' : visibility > 5 ? '🙂' : '😐'}</span>
          </div>
        </div>

        <div className="bg-gradient-to-br from-gray-50 to-white p-6 rounded-2xl">
          <div className="text-gray-600 text-sm mb-4">Qualité de l'air</div>
          <div className="text-5xl font-bold text-gray-800 mb-2">{airQualityIndex || 'N/A'}</div>
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-600">
              {airQualityIndex <= 50 ? 'Bonne' : airQualityIndex <= 100 ? 'Modérée' : 'Malsaine'}
            </span>
            <span>{airQualityIndex <= 50 ? '😊' : airQualityIndex <= 100 ? '😐' : '😷'}</span>
          </div>
        </div>
      </div>
      </div>
